...
ProcessList.prototype.outputResults = function() {
	...
	for (var k in this.processes) {
		...
		result_rows += '<tr><td><b>P'+ proc.id +'</b></td><td>'+ proc.burst +'</td><td>'+ proc.arrival +'</td><td>'+ wait +'</td><td>'+ turn +'</td></tr>';
	}
	result_rows += '<tr><td colspan="5">&nbsp;</td></tr>';
	result_rows += '<tr class="bold"><td>Average</td><td>'+ round_time_result(burst_sum / this.count) +'</td><td>-</td><td>'+ round_time_result(wait_sum / this.count) +'</td><td>'+ round_time_result(turn_sum / this.count) +'</td></tr>';
