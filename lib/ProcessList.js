...
ProcessList.prototype.outputResults = function() {
	...
	var result_rows = '<tr class="bold"><td>Processes</td><td>Arrival time</td><td>Burst time</td><td>Waiting time</td><td>Turnaround time</td></tr>';
	var wait_sum = 0;
	var turn_sum = 0;
	var burst_sum = 0;
	for (var k in this.processes) {
		...
		result_rows += '<tr><td><b>P'+ proc.id +'</b></td><td>'+ proc.arrival +'</td><td>'+ proc.burst+'</td><td>'+ wait +'</td><td>'+ turn +'</td></tr>';
	}
	result_rows += '<tr><td colspan="5">&nbsp;</td></tr>';
	result_rows += '<tr class="bold"><td>Average</td><td>-</td><td>'+ round_time_result(burst_sum / this.count) +'</td><td>'+ round_time_result(wait_sum / this.count) +'</td><td>'+ round_time_result(turn_sum / this.count) +'</td></tr>';
