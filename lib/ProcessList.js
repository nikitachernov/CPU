...
ProcessList.prototype.time_till_interrupt = function(current_process) {
    ...
    for (var i in this.processes) {
        ...
        if (process.priority == current_process.priority
            ...
            && process.last_pause < (time.time + current_process.remaining_burst)
            && process.remaining_burst < current_process.remaining_burst - (process.last_pause - time.time)) {
            if (process.last_pause < shortest_process_arrival)
                shortest_process_arrival = process.last_pause;
        }
    }
    console.log(shortest_process_arrival);
    return shortest_process_arrival - time.time;
}
