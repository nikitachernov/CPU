...
ProcessList.prototype.outputResults = function() {
	...
	for (var k in this.processes) {
		...
		wait_sum += wait;
		turn_sum += turn;
		result_rows += '<tr><td><b>P'+ proc.id +'</b></td><td>'+ wait +'</td><td>'+ turn +'</td></tr>';
	}
	result_rows += '<tr><td colspan="3">&nbsp;</td></tr>';
	result_rows += '<tr class="bold"><td>Average</td><td>'+ round_time_result(wait_sum / this.count) +'</td><td>'+ round_time_result(turn_sum / this.count) +'</td></tr>';
