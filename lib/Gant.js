...
Gant.prototype.outputResults = function() {
    ...
    for (var k in process_list.processes) {
		...
		if (first_proc) {
			result_rows = '<thead><tr class="bold"><th>Processes</th>';
			if (proc.priority != undefined) {
				result_rows += '<th>Priority</th>';
			}
			result_rows += '<th>Arrival time</th><th>Burst time</th><th>Waiting time</th><th>Turnaround time</th></tr></thead>';
			result_rows += '<tbody>';
			first_proc = false;
		}
		result_rows += '<tr><td class="middle" style="background-color: #' + proc.color + '"><b>P' + proc.id + '</b></td>';
		if (proc.priority != undefined) {
			result_rows += '<td>'+ proc.priority +'</td>';
		}
        result_rows += '<td>' + proc.arrival + '</td><td>' + proc.burst + '</td><td>' + wait + '</td><td>' + turn + '</td></tr>';
    }
    result_rows += '</tbody>'
    $("h2.results").show();
