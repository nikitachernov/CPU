...
Gant.prototype.push_empty = function(index, start_time, end_time) {
...
Gant.prototype.push_process = function(index, id, start_time, end_time, color) {
    this.gant_result[index] = {
        'type': 'proc',
        'id': id,
        'start': start_time,
...
Gant.prototype.initiate_or_continue_process = function(type, id, color) {
    ...
    case 'empty':
        if (previous_process['type'] == 'empty') {
            ...
            this.push_empty(time.time, time.time, time.time + 1);
        }
        break;
    case 'proc':
        if (previous_process['id'] == id) {
            // If current process is current - write into previous attribute
            this.gant_result[previous_process['start']]['end'] = (time.time + 1);
        } else {
            // Make new
            this.push_process(time.time, id, time.time, time.time + 1, color);
        }
        break;
    }
}
