...
Gant.prototype.outputResults = function() {
	...
	if (this.table_init === false) {
		this.table_init = true;
		$('#result_tbl').tablesorter({
			widgets: ['zebra'],
			sortList: [[(priority_tbl === true ? 5 : 4), 0]]
		});
		$('#result_tbl').html(thead + tfoot + tbody);
		$('#result_tbl').show();
	} else {
		$("#result_tbl tbody tr").addClass("delete");
		$('#result_tbl').html(thead + tfoot + tbody);
		$("#result_tbl").trigger("update");
		$("#result_tbl").trigger("appendCache");
		$("#result_tbl").trigger("sortList", [[(priority_tbl === true ? 5 : 4), 0]]);
		$("#result_tbl tbody tr.delete").remove();
		$("#result_tbl").trigger("update");
